...
/**
 * This is the mule business component that removes a splitting rule from Ax.
 *
 * @since 2012-06-12
 */

@Component
public class MuleSplittingRuleDeleted {

    private final static Logger log = LoggerFactory.getLogger(MuleSplittingRuleDeleted.class);
    ...
    public ServiceResult execute(ServiceResult serviceResult) {
        ...
        try {
            if (serviceResultIsOk(serviceResult)) {
                serviceResult = service.deleteSplittingRule(serviceResult);
            }
        } catch (BillingServiceException bse) {
            serviceResult = ServiceResultErrorHandler.handleBillingException(serviceResult, bse, "SplittingRuleDeleted WebService failed");
        } catch (Exception rex) {
