package com.tradedoubler.billing.service.ax.webservice.routingservice;

import com.tradedoubler.webservice.routing.clientagreement.RoutingService;

import java.net.URL;

/**
 * Extending class used for clarity to avoid the presence of multiple,
 * different, auto-generated RoutingService classes in the AxWebServiceSetup.
 */
public class AxSalesClientAgreementService extends RoutingService {

    public AxSalesClientAgreementService(URL wsdlLocation) {
        super(wsdlLocation);
    }

}
