package com.tradedoubler.webservice.routing.salescustomer;

import java.net.MalformedURLException;
import java.net.URL;
import javax.xml.namespace.QName;
import javax.xml.ws.WebEndpoint;
import javax.xml.ws.WebServiceClient;
import javax.xml.ws.WebServiceFeature;
import com.microsoft.schemas.dynamics._2008._01.services.CustomerService;
import com.microsoft.schemas.netfx._2009._05.routing.IRequestReplyRouter;
import javax.xml.ws.Service;

/**
 * This class was generated by Apache CXF 2.6.1
 * 2013-03-20T13:23:21.182+01:00
 * Generated source version: 2.6.1
 * 
 */
@WebServiceClient(name = "RoutingService", 
                  wsdlLocation = "http://tdaxaost01/MicrosoftDynamicsAXAif60/MDSITGSalesCust/xppservice.svc?wsdl",
                  targetNamespace = "http://tempuri.org/") 
public class RoutingService extends Service {

    public final static URL WSDL_LOCATION;

    public final static QName SERVICE = new QName("http://tempuri.org/", "RoutingService");
    public final static QName BasicHttpBindingCustomerService = new QName("http://tempuri.org/", "BasicHttpBinding_CustomerService");
    public final static QName ReqReplyEndpoint = new QName("http://tempuri.org/", "reqReplyEndpoint");
    static {
        URL url = null;
        try {
            url = new URL("http://tdaxaost01/MicrosoftDynamicsAXAif60/MDSITGSalesCust/xppservice.svc?wsdl");
        } catch (MalformedURLException e) {
            java.util.logging.Logger.getLogger(RoutingService.class.getName())
                .log(java.util.logging.Level.INFO, 
                     "Can not initialize the default wsdl from {0}", "http://tdaxaost01/MicrosoftDynamicsAXAif60/MDSITGSalesCust/xppservice.svc?wsdl");
        }
        WSDL_LOCATION = url;
    }

    public RoutingService(URL wsdlLocation) {
        super(wsdlLocation, SERVICE);
    }

    public RoutingService(URL wsdlLocation, QName serviceName) {
        super(wsdlLocation, serviceName);
    }

    public RoutingService() {
        super(WSDL_LOCATION, SERVICE);
    }
    

    /**
     *
     * @return
     *     returns CustomerService
     */
    @WebEndpoint(name = "BasicHttpBinding_CustomerService")
    public CustomerService getBasicHttpBindingCustomerService() {
        return super.getPort(BasicHttpBindingCustomerService, CustomerService.class);
    }

    /**
     * 
     * @param features
     *     A list of {@link javax.xml.ws.WebServiceFeature} to configure on the proxy.  Supported features not in the <code>features</code> parameter will have their default values.
     * @return
     *     returns CustomerService
     */
    @WebEndpoint(name = "BasicHttpBinding_CustomerService")
    public CustomerService getBasicHttpBindingCustomerService(WebServiceFeature... features) {
        return super.getPort(BasicHttpBindingCustomerService, CustomerService.class, features);
    }
    /**
     *
     * @return
     *     returns IRequestReplyRouter
     */
    @WebEndpoint(name = "reqReplyEndpoint")
    public IRequestReplyRouter getReqReplyEndpoint() {
        return super.getPort(ReqReplyEndpoint, IRequestReplyRouter.class);
    }

    /**
     * 
     * @param features
     *     A list of {@link javax.xml.ws.WebServiceFeature} to configure on the proxy.  Supported features not in the <code>features</code> parameter will have their default values.
     * @return
     *     returns IRequestReplyRouter
     */
    @WebEndpoint(name = "reqReplyEndpoint")
    public IRequestReplyRouter getReqReplyEndpoint(WebServiceFeature... features) {
        return super.getPort(ReqReplyEndpoint, IRequestReplyRouter.class, features);
    }

}
